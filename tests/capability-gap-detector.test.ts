...
test('capability-gap classifier: react-vite-ts ships lucide-react (orchestration, not gap)', () => {
  ...
  assert.ok(deps.has('lucide-react'), `react-vite-ts must ship lucide-react (blueprint-agent bug #4 orchestration class)`)
})
...
test('capability-gap classifier: no family ships snarkjs or circomlibjs (real scaffold gap)', () => {
  ...
  for (const pkg of ['snarkjs', 'circomlibjs']) {
    ...
    assert.deepEqual(shippers, [], `${pkg} appears in ${shippers.join(', ')} — gap-detector regression test stale. If you intentionally added ${pkg}, update this test.`)
  }
})
