...
export function verifyLockMatches(
  lock: ComposeLockFile,
  actualHashes: Record<string, string>,
): {
  ok: boolean
  missing: string[]
  extra: string[]
  differing: { path: string; expected: string; actual: string }[]
} {
  const expected = lock.fileHashes
  const missing: string[] = []
  const extra: string[] = []
  const differing: { path: string; expected: string; actual: string }[] = []
