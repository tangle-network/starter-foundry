...
app.onError((error, context) => {
  ...
  return context.json({ error: 'Agent API request failed' }, 500)
})
...
app.post('/api/prompt', async (context) => {
  ...
  return stream(context, async (output) => {
    ...
    try {
      for await (const event of sandbox.streamPrompt(prompt, { signal: controller.signal })) {
        await output.writeln(JSON.stringify(event))
      }
    } catch (error) {
      if (controller.signal.aborted) return
      console.error('Agent prompt stream failed', error)
      await output.writeln(
        JSON.stringify({ type: 'error', data: { message: 'Agent prompt stream failed' } }),
      )
    }
  })
})
...
const server = serve({ fetch: app.fetch, port, hostname }, ({ port: activePort }) => {
  console.log(`Agent app listening on http://${hostname}:${activePort}`)
})
