 ...
 * `benjaminiHochberg` from agent-eval@0.13.
 */
...
test('PROMOTE on a clear positive delta with adequate effect size', () => {
  ...
  const baseline = makeBatch([0.50, 0.52, 0.48, 0.51, 0.49, 0.53, 0.50, 0.48])
  const candidate = makeBatch([0.90, 0.92, 0.88, 0.91, 0.89, 0.93, 0.90, 0.88])
  const decision = gate.evaluate(candidate, baseline)
  assert.equal(decision.verdict, 'PROMOTE', `expected PROMOTE got ${decision.verdict}: ${decision.reason}`)
...
test('REVERT on overfit (held-out gap exceeds threshold)', () => {
  ...
  const baseline = makeBatch([0.50, 0.52, 0.48, 0.51, 0.49, 0.53])
  // Candidate scores 0.95 on search but only 0.5 on held-out → overfit gap = 0.45
  const candidate = makeBatch(
    [0.95, 0.95, 0.95, 0.95, 0.95, 0.95],
    { holdout: [0.50, 0.50, 0.50, 0.50, 0.50, 0.50] },
  )
  const decision = gate.evaluate(candidate, baseline)
  assert.equal(decision.verdict, 'REVERT', `expected REVERT got ${decision.verdict}: ${decision.reason}`)
...
test('REVERT on significantly worse paired-delta', () => {
  const gate = new HeldOutGate({ baselineKey: 'b', seed: 1 })
  const baseline = makeBatch([0.90, 0.92, 0.88, 0.91, 0.89, 0.93, 0.90, 0.88])
  const candidate = makeBatch([0.50, 0.52, 0.48, 0.51, 0.49, 0.53, 0.50, 0.48])
  const decision = gate.evaluate(candidate, baseline)
  assert.equal(decision.verdict, 'REVERT', `expected REVERT got ${decision.verdict}: ${decision.reason}`)
  assert.ok(decision.evidence.pairedDeltaMedian < 0)
})
...
test('HOLD when delta exists but Cohen\'s d below threshold', () => {
  ...
  const baseline = makeBatch([0.10, 0.30, 0.50, 0.70, 0.90, 0.45, 0.55, 0.65])
  const candidate = makeBatch([0.20, 0.40, 0.60, 0.80, 0.95, 0.55, 0.65, 0.75])
  const decision = gate.evaluate(candidate, baseline)
  assert.equal(decision.verdict, 'HOLD', `expected HOLD got ${decision.verdict}: ${decision.reason}`)
...
test('BH correction applies q-value when applyBHCorrection: true', () => {
  ...
  const baseline = makeBatch([0.50, 0.52, 0.48, 0.51, 0.49, 0.53])
  const candidate = makeBatch([0.90, 0.92, 0.88, 0.91, 0.89, 0.93])
  const decision = gate.evaluate(candidate, baseline)
