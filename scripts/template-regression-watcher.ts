...
//   node scripts/template-regression-watcher.ts --open-pr
// Local inspection:
//   node scripts/template-regression-watcher.ts
    ...
    [
      ...
      [
        ...
        [
          ...
          ``,
          `If the regression is spurious (e.g. unrelated traffic spike), close this PR and increase REGRESSION_THRESHOLD in \`scripts/template-regression-watcher.ts\`.`,
          ``,
          `_Opened by \`scripts/template-regression-watcher.ts\`._`,
        ].join('\n'),
      ],
    ],
    ['git', ['checkout', '-']],
