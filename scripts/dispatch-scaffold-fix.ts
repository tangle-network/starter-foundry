...
//
// Usage:
//   node scripts/dispatch-scaffold-fix.ts                    # picks top cluster
...
import { dispatchAgenticProposal } from './_lib/agentic-proposer.ts'
...
if (!existsSync(FEEDBACK)) {
  console.error('no consumer feedback — run scripts/consume-vb-feedback.ts first')
  process.exit(2)
}
