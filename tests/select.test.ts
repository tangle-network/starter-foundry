...
test('select chooses typescript agent starter for langgraph prompts', async () => {
  const result = await selectStarter({
    prompt:
      'Build an AI agent service in TypeScript using LangGraph with tool execution and memory.',
    partner: null,
  })
...
test('select chooses hardhat starter for explicit hardhat prompts', async () => {
  const result = await selectStarter({
    prompt:
      'Create a Hardhat TypeScript project configured for X Layer with deploy task and contract verification.',
    partner: null,
  })
...
test('select chooses evm infra starter for chain monitoring prompts', async () => {
  const result = await selectStarter({
    prompt:
      'Build a viem-based WebSocket block monitor for X Layer that exposes a stats JSON endpoint',
    partner: null,
  })
