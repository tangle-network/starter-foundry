...
export function buildWebProject(
  ...
  if (hasAny(text, ['three.js', 'threejs', 'webgl 3d scene', '3d model viewer', 'gltf viewer', '3d scene editor'])) {
    return {
      id: 'web',
      path: 'apps/web',
      spec: {
        ...
        variables: {
          ...
          subheadline: 'Three.js + Vite starter with renderer, camera, and resize loop wired from the first render.',
        },
    ...
    return {
      ...
      spec: {
        ...
        variables: {
          ...
          subheadline: 'Pixi v8 + Vite starter with async Application init + WebGPU-preferred renderer.',
        },
        primaryArtifactTargetMs: 3000,
      },
    }
  ...
  if (hasAny(text, ['eleventy', '11ty', '@11ty', 'nunjucks layout'])) {
    return {
      ...
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'eleventy-static', layers: ['framework:eleventy-static'],
        partner: resolvePartnerForFamily(partner, 'eleventy-static'),
        slots: {},
        variables: { headline: 'Content in minutes with 11ty', subheadline: 'Eleventy + Nunjucks layouts, no client JS by default.' },
  ...
  if (hasAny(text, ['hugo site', 'hugo static', 'hugo blog', 'hugo layout'])) {
    return {
      ...
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'hugo-static', layers: ['framework:hugo-static'],
        partner: resolvePartnerForFamily(partner, 'hugo-static'),
        slots: {},
        variables: { headline: 'Hugo — fast, binary-driven static sites', subheadline: 'Go-based SSG with Tera templates and sub-second builds.' },
        primaryArtifactTargetMs: 2500,
      },
    }
  }
  if (hasAny(text, ['zola', 'zola site', 'zola static', 'tera template'])) {
    return {
      ...
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'zola-static', layers: ['framework:zola-static'],
        partner: resolvePartnerForFamily(partner, 'zola-static'),
        slots: {},
        variables: { headline: 'Zola — Rust-native SSG', subheadline: 'Single-binary static site generator with built-in SASS.' },
        primaryArtifactTargetMs: 2500,
      },
    }
  }
  if (hasAny(text, ['jupyter book', 'jupyterbook', 'myst-nb', 'executable book'])) {
    return {
      ...
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'jupyter-book', layers: ['framework:jupyter-book'],
        partner: resolvePartnerForFamily(partner, 'jupyter-book'),
        slots: {},
        variables: { headline: 'Executable book', subheadline: 'Jupyter Book with MyST code cells.' },
        primaryArtifactTargetMs: 3500,
      },
    }
  }
  if (hasAny(text, ['observable framework', 'observable notebook', 'observablehq', 'observable plot'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'observable-notebook', layers: ['framework:observable-notebook'],
        partner: resolvePartnerForFamily(partner, 'observable-notebook'),
        slots: {},
        variables: { headline: 'Data stories with Observable', subheadline: 'Observable Framework + Plot charts, Markdown-first.' },
        primaryArtifactTargetMs: 3000,
      },
    }
  }
  ...
  if (hasAny(text, ['streamlit multipage', 'multipage streamlit', 'production streamlit', 'streamlit dashboard app'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'streamlit-advanced', layers: ['framework:streamlit-advanced'],
        partner: resolvePartnerForFamily(partner, 'streamlit-advanced'),
        slots: {},
        variables: { headline: 'Production Streamlit', subheadline: 'Multipage + caching + Plotly.' },
        primaryArtifactTargetMs: 3500,
      },
    }
  }
  ...
  if (hasAny(text, ['webgpu compute', 'webgpu inference', 'wgsl compute', 'browser ml', 'transformers.js'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'webgpu-inference', layers: ['framework:webgpu-inference'],
        partner: resolvePartnerForFamily(partner, 'webgpu-inference'),
        slots: {},
        variables: { headline: 'GPU compute in the browser', subheadline: 'Raw WebGPU matmul on Float32 buffers.' },
        primaryArtifactTargetMs: 3000,
      },
    }
  }
  if (hasAny(text, ['webgpu rendering', 'raw webgpu', 'wgsl graphics', 'wgsl vertex', 'low-level gpu graphics', 'webgpu triangle'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'webgpu-render', layers: ['framework:webgpu-render'],
        partner: resolvePartnerForFamily(partner, 'webgpu-render'),
        slots: {},
        variables: { headline: 'Raw WebGPU renderer', subheadline: 'WGSL vertex + fragment, no three.js.' },
        primaryArtifactTargetMs: 3000,
      },
    }
  }
  ...
  if (hasAny(text, ['bevy', 'bevy web', 'rust web game', 'wasm game engine'])) {
    return {
      ...
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'bevy-web', layers: ['framework:bevy-web'],
        partner: resolvePartnerForFamily(partner, 'bevy-web'),
        slots: {},
        variables: { headline: 'Bevy + Trunk', subheadline: 'Rust game engine compiled to WASM for the browser.' },
  ...
  if (hasAny(text, ['godot', 'godot web', 'godot html5', 'godot 4', 'gdscript'])) {
    return {
      ...
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'godot-web', layers: ['framework:godot-web'],
        partner: resolvePartnerForFamily(partner, 'godot-web'),
        slots: {},
        variables: { headline: 'Godot 4 on the web', subheadline: 'HTML5 export with COOP/COEP headers wired.' },
        primaryArtifactTargetMs: 4000,
      },
    }
  }
  if (hasAny(text, ['unity webgl', 'unity web build', 'unity batch mode', 'unity html5'])) {
    return {
      ...
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'unity-web-proxy', layers: ['framework:unity-web-proxy'],
        partner: resolvePartnerForFamily(partner, 'unity-web-proxy'),
        slots: {},
        variables: { headline: 'Unity WebGL host', subheadline: 'Serving shell + batch-mode build scripts.' },
        primaryArtifactTargetMs: 4000,
      },
    }
  }
  ...
  if (hasAny(text, ['skia', 'react native skia', 'reanimated', 'rich animations mobile', 'native gestures mobile', 'expo skia', 'expo reanimated'])) {
    return {
      id: 'web', path: 'apps/mobile',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-mobile`,
        family: 'expo-rn-rich', layers: ['framework:expo-rn-rich'],
        partner: resolvePartnerForFamily(partner, 'expo-rn-rich'),
        slots: {},
        variables: { headline: 'Mobile app with Skia + Reanimated', subheadline: 'Rich GPU animations + gestures from frame 1.' },
        primaryArtifactTargetMs: 3500,
      },
    }
  }
  ...
  if (hasAny(text, ['menu bar app', 'macos menubar', 'floating panel app', 'raycast-like', 'menubar popover'])) {
    return {
      id: 'web', path: 'apps/desktop',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-desktop`,
        family: 'tauri-menubar', layers: ['framework:tauri-menubar'],
        partner: resolvePartnerForFamily(partner, 'tauri-menubar'),
        slots: {},
        variables: { headline: 'A compact panel in the menu bar', subheadline: 'Tauri popover tuned for Raycast-like flows.' },
        primaryArtifactTargetMs: 3500,
      },
    }
  }
  if (hasAny(text, ['system tray app', 'tray icon only', 'background daemon', 'tauri tray', 'tray daemon'])) {
    return {
      id: 'web', path: 'apps/desktop',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-desktop`,
        family: 'tauri-tray', layers: ['framework:tauri-tray'],
        partner: resolvePartnerForFamily(partner, 'tauri-tray'),
        slots: {},
        variables: { headline: 'Tray-only daemon', subheadline: 'No primary window — icon + menu + notifications.' },
        primaryArtifactTargetMs: 3500,
      },
    }
  }
  if (hasAny(text, ['native os integration', 'custom protocol handler', 'deep links desktop', 'os notifications desktop', 'auto updater electron', 'electron native'])) {
    return {
      id: 'web', path: 'apps/desktop',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-desktop`,
        family: 'electron-native-os', layers: ['framework:electron-native-os'],
        partner: resolvePartnerForFamily(partner, 'electron-native-os'),
        slots: {},
        variables: { headline: 'Native OS integrations in Electron', subheadline: 'Menus, protocols, deep links, auto-update.' },
        primaryArtifactTargetMs: 3500,
      },
    }
  }
  ...
  if (hasAny(text, ['voice agent', 'voice first', 'conversational voice', 'speech to text agent', 'browser mic agent'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'voice-first-agent', layers: ['framework:voice-first-agent'],
        partner: resolvePartnerForFamily(partner, 'voice-first-agent'),
        slots: {},
        variables: { headline: 'Talk to the product', subheadline: 'Browser mic → STT → LLM → TTS loop.' },
        primaryArtifactTargetMs: 3000,
      },
    }
  }
  if (hasAny(text, ['vision agent', 'camera agent', 'visual qa', 'screen understanding', 'ocr agent'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'vision-first-agent', layers: ['framework:vision-first-agent'],
        partner: resolvePartnerForFamily(partner, 'vision-first-agent'),
        slots: {},
        variables: { headline: 'See what the user sees', subheadline: 'Camera → multimodal LLM → overlay.' },
        primaryArtifactTargetMs: 3000,
      },
    }
  }
  if (hasAny(text, ['multimodal agent', 'multimodal input', 'text image audio', 'gpt-4o client', 'claude vision voice', 'multimodal chat'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'multimodal-agent', layers: ['framework:multimodal-agent'],
        partner: resolvePartnerForFamily(partner, 'multimodal-agent'),
        slots: {},
        variables: { headline: 'Text + image + audio, one pane', subheadline: 'Unified multipart POST to a multimodal endpoint.' },
        primaryArtifactTargetMs: 3000,
      },
    }
  }

  if (hasAny(text, ['realtime audio', 'webaudio visualiz', 'webaudio analyser', 'peer audio', 'webrtc audio'])) {
    return {
      id: 'web', path: 'apps/web',
      spec: {
        projectName: `${buildSlug(prompt, 'workspace')}-web`,
        family: 'realtime-audio-ts', layers: ['framework:realtime-audio-ts'],
        partner: resolvePartnerForFamily(partner, 'realtime-audio-ts'),
        slots: {},
        variables: { headline: 'Live audio on canvas', subheadline: 'WebAudio + analyser + WebRTC peer seam.' },
        primaryArtifactTargetMs: 3000,
      },
    }
  }
    ...
    return {
      ...
      spec: {
        ...
        variables: {
          ...
          subheadline: 'Astro 5 static starter — islands architecture, file-based routing, MDX-ready.',
        },
        primaryArtifactTargetMs: 2500,
      },
    }
...
export function chooseApiFamily(text: string): FamilyChoice {
  ...
  if (hasAny(text, ['rag pipeline', 'retrieval augmented', 'chroma vector', 'sentence-transformers', 'semantic retrieval', 'rag chroma'])) {
    return { family: 'rag-pipeline-py', layers: ['framework:rag-pipeline-py'], path: 'apps/inference' }
  }
  // Inference-serving variants that must beat vllm-server's generic terms.
  if (hasAny(text, ['tgi', 'text generation inference', 'huggingface tgi', 'hf inference server'])) {
  ...
  if (hasAny(text, ['triton inference', 'nvidia triton', 'triton server', 'model repository triton'])) {
  ...
  if (hasAny(text, ['skypilot', 'sky serve', 'multi-cloud llm', 'spot inference', 'sky.yaml'])) {
    return { family: 'skypilot-serving', layers: ['framework:skypilot-serving'], path: 'apps/inference' }
  }
  if (hasAny(text, ['lora fine-tune', 'lora training', 'qlora', 'peft adapter', 'fine-tune llm', 'sft training'])) {
    return { family: 'lora-training', layers: ['framework:lora-training'], path: 'apps/inference' }
  }

  // Media / realtime infrastructure.
  if (hasAny(text, ['livekit sfu', 'self-hosted livekit', 'selective forwarding unit', 'webrtc sfu', 'livekit server self-hosted'])) {
    return { family: 'livekit-sfu', layers: ['framework:livekit-sfu'], path: 'apps/api' }
  }
  if (hasAny(text, ['hls origin', 'rtmp ingest', 'm3u8 origin', 'hls live streaming', 'live streaming origin'])) {
    return { family: 'hls-origin', layers: ['framework:hls-origin'], path: 'apps/api' }
  }

  // Chain-specific families.
  if (hasAny(text, ['celestia', 'data availability node', 'blobstream', 'celestia light node', 'celestia blob'])) {
    return { family: 'celestia-da', layers: ['framework:celestia-da'], path: 'apps/api' }
  }

  // Industry-vertical APIs — opinionated schemas.
  if (hasAny(text, ['hipaa', 'phi audit', 'patient records', 'healthcare backend', 'healthcare compliance', 'medical records'])) {
    return { family: 'healthcare-hipaa-backend', layers: ['framework:healthcare-hipaa-backend'], path: 'apps/api' }
  }
  if (hasAny(text, ['double entry', 'double-entry', 'ledger backend', 'fintech accounting', 'debit credit', 'financial ledger'])) {
    return { family: 'fintech-ledger-backend', layers: ['framework:fintech-ledger-backend'], path: 'apps/api' }
  }
  if (hasAny(text, ['legal case', 'case management', 'matter management', 'law practice', 'attorney timekeeping', 'legal billing'])) {
    return { family: 'legal-case-mgmt', layers: ['framework:legal-case-mgmt'], path: 'apps/api' }
  }
  if (hasAny(text, ['k-12', 'k12', 'edtech', 'classroom management', 'gradebook', 'student information system'])) {
    return { family: 'k12-edtech', layers: ['framework:k12-edtech'], path: 'apps/api' }
  }
  if (hasAny(text, ['crm backend', 'sales pipeline', 'sales crm', 'customer relationship', 'deals pipeline'])) {
    return { family: 'crm-backend', layers: ['framework:crm-backend'], path: 'apps/api' }
  }
  if (hasAny(text, ['headless commerce', 'ecommerce backend', 'cart api', 'checkout api', 'product catalog api', 'order management api'])) {
    return { family: 'ecommerce-headless', layers: ['framework:ecommerce-headless'], path: 'apps/api' }
  }
  ...
  if (hasAny(text, ['ollama', 'ollama local', 'local llm', 'gguf', 'llama.cpp', 'on-device llm', 'self-hosted inference', 'ollama modelfile'])) {
