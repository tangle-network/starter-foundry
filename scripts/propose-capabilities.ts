...
if (!existsSync(buildoutsPath)) { console.error('no buildouts.jsonl — run run-buildout-pipeline first'); process.exit(2) }
const capMap = JSON.parse(readFileSync(join(REPO, 'registry/package-to-capability.json'), 'utf8')).mapping

const events = readFileSync(buildoutsPath, 'utf8').split('\n').filter(Boolean).map((l) => { try { return JSON.parse(l) } catch { return null } }).filter(Boolean)
...
for (const [pkg, agg] of unmappedCooccurrence) {
  ...
  const cooc = [...agg.cooccursWith.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5).map(([n, c]) => ({ name: n, count: c }))
  const proposalId = pkg.replace(/[^a-z0-9-]/gi, '-').replace(/^-+|-+$/g, '').toLowerCase().slice(0, 40) || 'proposal'
  const proposal = {
  ...
  writeFileSync(join(OUT_DIR, `${proposal.proposedCapabilityId}.json`), JSON.stringify(proposal, null, 2))
  proposals.push(proposal)
}

console.log(`✓ emitted ${proposals.length} capability proposals → ${OUT_DIR}`)
for (const p of proposals.slice(0, 10)) {
  console.log(`  ${p.triggerPackage.padEnd(32)} ${String(p.timesInstalled).padStart(3)}× across ${p.scenarios.length} scenarios (fail rate ${(p.failureRate * 100).toFixed(0)}%)`)
}
