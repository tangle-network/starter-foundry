...
test('detectLane: zk', () => {
  ...
  assert.ok(!detectLane('private voting with FHE encrypted ballots', 'zk'))
...
test('detectIndustry: fitness prompt', async () => {
  ...
  assert.equal(
    detectIndustry('Build a marathon training app for runners', 'react-vite-ts', r),
    'industry:fitness',
  )
})
...
test('detectIndustry: crypto prompt', async () => {
  const r = await loadRegistry()
  assert.equal(
    detectIndustry('Build a defi staking dashboard with token rewards', 'react-vite-ts', r),
    'industry:crypto',
  )
})
...
test('detectIndustry: restaurant prompt', async () => {
  const r = await loadRegistry()
  assert.equal(
    detectIndustry('Build a restaurant menu and reservations site', 'react-vite-ts', r),
    'industry:restaurant',
  )
})
...
test('detectCapabilities: tiered capability keywords must be high-signal', async () => {
  const r = await loadRegistry()
  const contractDashboard = detectCapabilities(
    'Build a React Vite TypeScript dashboard for a Fhenix Foundry contract',
    'react-vite-ts',
    r,
  )
  assert.ok(!contractDashboard.includes('capability:evm-nft-mint-page'))
  assert.ok(!contractDashboard.includes('capability:zk-browser'))

  const nftMint = detectCapabilities(
    'Build an NFT mint page with wallet connect and an OpenSea link',
    'react-vite-ts',
    r,
  )
  assert.ok(nftMint.includes('capability:evm-nft-mint-page'))

  const browserZk = detectCapabilities(
    'Build anonymous voting with a nullifier and Merkle proof in the browser',
    'react-vite-ts',
    r,
  )
  assert.ok(browserZk.includes('capability:zk-browser'))
})
