...
import { createTempDir, removeDir, writeJson } from "./fs.mjs";
...
function summarize(values) {
  ...
  const pick = (ratio) => ordered[Math.min(ordered.length - 1, Math.floor(ratio * (ordered.length - 1)))];
...
export async function benchmarkStarter({ spec, runs = 1, outDir = null }) {
  const results = [];
