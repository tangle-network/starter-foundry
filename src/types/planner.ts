 ...
 * matched a family by score >0. `ambiguous` means the prompt matched a known
 * ambiguity group but did not name enough provider/runtime/protocol detail to
 * safely choose one pack. `fallback-product` means score=0 but the prompt
 * looked like a natural-language product description, so the planner defaulted
 * to `fullstack-ts`. `fallback-static` means score=0 and we picked
...
export interface SelectionResult {
  ...
  reasons: string[]
  /**
   * Categorical signal for non-ideal paths. `safe` = score>0 match.
   * `ambiguous` = known domain-pack ambiguity; caller should disambiguate
   * provider/runtime/protocol before using the placeholder spec.
   * `fallback-*` = score=0 with reason. `unrouteable` = score=0 AND prompt is
   * a technical identifier (kebab-case clusters of nouns) — caller MUST
