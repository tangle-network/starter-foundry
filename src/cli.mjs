...
import { createAuditBundle } from "./lib/audit.mjs";
import { evaluateAgents } from "./lib/agent-runners.mjs";
import { benchmarkStarter } from "./lib/benchmark.mjs";
import { buildCatalog } from "./lib/catalog.mjs";
import { composeStarter } from "./lib/compose.mjs";
import { createContextPack } from "./lib/context-pack.mjs";
import { readJson } from "./lib/fs.mjs";
import { runPromptCorpus } from "./lib/prompt-e2e.mjs";
import { planPrompt } from "./lib/prompt-planner.mjs";
import { runProofSuite } from "./lib/prove.mjs";
import { listRegistry, loadProjectSpec } from "./lib/registry.mjs";
import { createRelease } from "./lib/release.mjs";
import { selectStarter } from "./lib/selection.mjs";
import { validateStarter } from "./lib/validate.mjs";
import { benchmarkWorkspace, composeWorkspace, createWorkspaceContextPack } from "./lib/workspace.mjs";
...
async function main() {
  ...
  switch (command) {
    ...
    case "compose": {
      ...
      const result = await composeStarter({
        spec,
    ...
    case "evaluate": {
      ...
      const result = await evaluateAgents({
        spec,
        outDir: options.out ? String(options.out) : null,
        agents: options.agents ? String(options.agents).split(",").map((value) => value.trim()).filter(Boolean) : null,
      });
