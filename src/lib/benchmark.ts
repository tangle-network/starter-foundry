...
import type { ComposeSpec, BenchmarkReport, BenchmarkStats, BenchmarkRunResult } from '../types.js'
...
function summarize(values: number[]): BenchmarkStats {
  ...
  const pick = (ratio: number) => ordered[Math.min(ordered.length - 1, Math.floor(ratio * (ordered.length - 1)))]!
