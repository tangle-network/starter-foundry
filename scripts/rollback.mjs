...
//   node scripts/rollback.mjs --to <version>            # dry-run
//   node scripts/rollback.mjs --to <version> --apply    # actually revert
...
if (LIST) {
  ...
  if (files.length === 0) {
    console.log('no snapshots recorded — run `node scripts/rollback.mjs --snapshot` after a release')
    process.exit(0)
  }
