...
//
// Usage:
//   node scripts/llm-buildout-diagnose.mjs           # runs LLM, writes reports
//   node scripts/llm-buildout-diagnose.mjs --dry-run # compute input only
//
...
if (!existsSync(ANALYSIS)) {
  console.log('no buildout-analysis.json — run scripts/run-buildout-pipeline.mjs first')
  process.exit(0)
}
