...
export async function batchExport({
  ...
  for (let i = 0; i < templateNames.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(
      batch.map((name) => exportSingle(name, mapping.templates[name]!, outDir, shouldFatten, shouldValidate, layerCompatibility)),
    )
...
async function exportSingle(
  ...
  try {
    ...
    if (!archived) {
      return { template: templateName, success: false, archivePath: null, filesWritten: composeResult.filesWritten.length, composeMs, fattenMs, validationOk, error: 'tar failed' }
    }
    ...
    return { template: templateName, success: true, archivePath, filesWritten: composeResult.filesWritten.length, composeMs, fattenMs, validationOk, error: null }
  } catch (err) {
    return { template: templateName, success: false, archivePath: null, filesWritten: 0, composeMs: 0, fattenMs: 0, validationOk: false, error: err instanceof Error ? err.message : String(err) }
  } finally {
    // Clean up compose dir to save disk
    if (!shouldFatten) {
      await fs.rm(path.join(outDir, '_compose', templateName), { recursive: true, force: true }).catch(() => {})
    }
  }
}
