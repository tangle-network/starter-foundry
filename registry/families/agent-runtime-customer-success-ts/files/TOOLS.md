# TOOLS — agent-runtime-customer-success-ts
...
| Tool | Intent | Notes |
|---|---|---|
| `retention-playbook` | Concrete tool implementing the `retention-playbook` capability | Operator implements when needed |
| `health-score` | Concrete tool implementing the `health-score` capability | Operator implements when needed |
| `escalation-triage` | Concrete tool implementing the `escalation-triage` capability | Operator implements when needed |
| `qbr-framework` | Concrete tool implementing the `qbr-framework` capability | Operator implements when needed |
