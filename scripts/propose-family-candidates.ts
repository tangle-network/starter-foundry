...
//
// Mirrors scripts/propose-capability-candidates.ts; see that file for the
...
//
// Usage:
//   node scripts/detect-family-gaps.ts --json --top 3 \
//     | node scripts/propose-family-candidates.ts
//   node scripts/propose-family-candidates.ts --mode=rlm --max-shots 2
//   node scripts/propose-family-candidates.ts --full-boot      # enable scaffold-runs
...
async function run() {
  ...
  console.log(`Next: run \`node scripts/promote-family-proposal.ts --all\` to validate + promote.`)
}
