...
try:
    ...
    from langchain.text_splitter import RecursiveCharacterTextSplitter  # type: ignore[assignment]
...
def retrieve(query: str, k: int = TOP_K) -> list[tuple[str, str, float]]:
    ...
    return [(d, m.get("source", "?"), float(dist)) for d, m, dist in zip(docs, metas, dists)]
