...
test('planPrompt attaches capability:zk-browser on mixer-style ZK prompts', async () => {
  const plan = await planPrompt({
    prompt: 'Build a privacy-preserving mixer UI where deposit generates a commitment and withdraw accepts a nullifier-preimage and generates a ZK proof client-side',
    partner: null,
  })
  if (plan.kind !== 'starter') return
  const layers = plan.spec.layers ?? []
  assert.ok(layers.includes('capability:zk-browser'), `expected zk-browser in layers, got: ${layers.join(', ')}`)
})

test('planPrompt attaches capability:zk-browser on private voting prompts', async () => {
  const plan = await planPrompt({
    prompt: 'Build an anonymous voting app where each vote carries a nullifier and merkle proof to prevent double-voting without revealing the voter',
    partner: null,
  })
  if (plan.kind !== 'starter') return
  const layers = plan.spec.layers ?? []
...
test('planPrompt does NOT attach zk-browser on plain ZK prover-service prompts', async () => {
  ...
  const plan = await planPrompt({
    prompt: 'Build a Rust ZK prover service using sp1 to generate proofs over Ethereum transactions',
    partner: null,
  })
  const allLayers = plan.kind === 'starter'
    ? (plan.spec.layers ?? [])
    : (plan.spec.projects ?? []).flatMap((p: { spec: { layers?: string[] } }) => p.spec.layers ?? [])
  assert.ok(
...
test('compose merges capability:zk-browser packageDeps into composed package.json', async () => {
  ...
  try {
    ...
    assert.ok(pkg.dependencies?.['snarkjs'], `composed package.json should include snarkjs; got deps: ${Object.keys(pkg.dependencies ?? {}).join(', ')}`)
    ...
    const helperExists = await fs.access(path.join(outDir, 'src/lib/zkproof.ts')).then(() => true).catch(() => false)
