...
on top of `@tangle-network/agent-eval@^0.13.0`. The eval-harness layers
(scenarios, judge-rubric, regression) supply the measurement
substrate; the auto-research layer wraps `OptimizationLoop` +
`runPromptEvolution` + `runProposeReview` + `paretoFrontier`.
