...
describe('declared-dep-used', () => {
  test('passes when every declared dep is imported somewhere', () => {
    ...
    try {
      writeFile(join(dir, 'package.json'), JSON.stringify({
        name: 'scaffold', dependencies: { 'pkg-a': '^1.0.0' },
      }))
  ...
  test('handles scoped package names correctly on pass', () => {
    ...
    try {
      writeFile(join(dir, 'package.json'), JSON.stringify({
        name: 'scaffold', dependencies: { '@scope/name': '^1.0.0' },
      }))
      writeFile(join(dir, 'src/lib.ts'), `import x from '@scope/name'\nexport { x }\n`)
      const manifest = { packageDeps: { dependencies: { '@scope/name': '^1.0.0' } } }
      const res = checkDeclaredDepUsed({ manifest, composedDir: dir })
      assert.equal(res.status, 'pass', `expected pass for scoped pkg, got ${res.status} — ${JSON.stringify(res)}`)
    } finally { rmSync(dir, { recursive: true, force: true }) }
  })
  ...
  test('root-level package.json declaration alone does not count as use', () => {
    ...
    try {
      writeFile(join(dir, 'package.json'), JSON.stringify({
        name: 'scaffold',
        dependencies: { '@tangle-network/agent-eval': '^0.7.0' },
      }))
      writeFile(join(dir, 'src/index.ts'), `export const x = 1\n`)
      const manifest = { packageDeps: { dependencies: { '@tangle-network/agent-eval': '^0.7.0' } } }
      const res = checkDeclaredDepUsed({ manifest, composedDir: dir })
...
`)
      ...
      assert.equal(res.status, 'pass', `expected pass — got status=${res.status} reason=${res.reason ?? ''} stderr=${res.stderr ?? ''}`)
  ...
  test('fails when start script exits immediately', async () => {
    const dir = scratchDir()
    try {
      writeFile(join(dir, 'boom.mjs'), `
console.error('failing immediately')
process.exit(1)
`)
      writeFile(join(dir, 'package.json'), JSON.stringify({
        name: 'boom-scaffold', scripts: { start: 'node boom.mjs' },
      }))
...
describe('eval-scores (integration)', () => {
  ...
  test('runs run-eval.mjs against booted agent and asserts aggregate >= threshold', async () => {
    ...
    assert.ok(true, 'enable by implementing the toolchain-warm path — see src/lib/promoter-gates.ts')
  })
})
