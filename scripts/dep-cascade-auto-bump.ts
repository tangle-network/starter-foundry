...
// by scripts/dep-cascade-detector.ts) and opens a SINGLE PR that bumps
...
//
// Usage:
//   node scripts/dep-cascade-auto-bump.ts                 # dry-run
//   node scripts/dep-cascade-auto-bump.ts --apply         # open PRs
//   node scripts/dep-cascade-auto-bump.ts --max 3 --apply # cap PR count
...
if (!existsSync(CASCADE_PATH)) {
  console.log('no dep-cascade.json — run scripts/dep-cascade-detector.ts first')
  process.exit(0)
}
  ...
  try {
    ...
    sh('gh', [
      ...
      [
        ...
        '',
        'Generated by `scripts/dep-cascade-auto-bump.ts`. Review layer-by-layer if the newest version introduces a breaking change; otherwise merge.',
      ].join('\n'),
    ])
