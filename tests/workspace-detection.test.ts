...
test("frontend + API + worker → workspace with 3 projects", async () => {
  const result = await planPrompt({
    prompt: "Build a trading platform with React dashboard, Node.js API backend, and a background worker for market streams",
    partner: null,
  });
  assert.equal(result.kind, "workspace");
  assert.ok(result.spec.projects.some((p) => p.id === "web"));
...
test("EVM + Solana multichain → workspace", async () => {
  const result = await planPrompt({
    prompt: "Build a cross-chain bridge with Solidity contracts on Ethereum and an Anchor program on Solana with a React frontend",
    partner: null,
  });
  assert.equal(result.kind, "workspace");
  assert.ok(result.spec.projects.some((p) => p.id === "evm"));
...
test("workspace without frontend uses first project as primary", async () => {
  const result = await planPrompt({
    prompt: "Build a workspace with a Node.js API backend and a trading bot worker with market streams",
    partner: null,
  });
  assert.equal(result.kind, "workspace");
